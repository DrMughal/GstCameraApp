 ...
 * */
   // if (ahc->state == GST_STATE_PLAYING)
    gst_rtsp_media_factory_set_launch ( ahc->factory, "( videotestsrc pattern=18 ! videoconvert ! videoscale ! video/x-raw,width=(int)480,height=(int)270, framerate=25/1, format=(string)I420 ! x264enc tune=zerolatency speed-preset=superfast !  rtph264pay name=pay0 pt=96 )");
