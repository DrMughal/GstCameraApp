...
//  ahc->filter = gst_element_factory_make ("capsfilter", NULL);
   ...
   * element with pay%d names will be a stream */
    ...
    ahc->pipeline= gst_parse_launch( " ahcsrc name=camera !  videoscale ! videoconvert ! video/x-raw, width=480, height=270, framerate=30/1 ! tee name=t ! queue  ! glimagesink name=vidsink t. !  intervideosink  channel=liveling    ", &err );
    if (err) {
 ...
 * */
   // if (ahc->state == GST_STATE_PLAYING)
    gst_rtsp_media_factory_set_launch ( ahc->factory, "(  intervideosrc do-timestamp=true channel=liveling  !  videoconvert ! video/x-raw, width=(int)480,height=(int)270, format=(string)I420, framerate=25/1 ! queue ! x264enc tune=zerolatency speed-preset=superfast !  rtph264pay name=pay0 pt=96  openslessrc  ! queue  ! audioconvert ! audio/x-raw, channels=1, depth=16, width=16, rate=16000 ! rtpL16pay name=pay1 pt=11  )");
