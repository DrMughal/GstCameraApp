/* Basic with Tee elements in  gst_rtsp_media_factory_set_launch and media-configure callback
 *
                 ...
                 GstAhc *user_data)
{
    ...
    user_data->vsink = gst_bin_get_by_name_recurse_up (GST_BIN (media_element), "vsink");
    user_data->ahcsrc = gst_bin_get_by_name_recurse_up (GST_BIN (media_element), "camera");
...
/*this profile ensures quicker re-syncing but streams cant be played by vlc of ffplay. */
...
*/
 // gst_element_link_many (ahc->ahcsrc, ahc->filter, ahc->vsink, NULL);
   ...
   //gst_rtsp_media_factory_set_launch ( ahc->factory, "( ahcsrc ! videoconvert ! videoscale ! video/x-raw,width=(int)480,height=(int)270,format=(string)I420 ! x264enc tune=zerolatency !  rtph264pay name=pay0 pt=96 )");

    /*With preview and Audio*/
    // gst_rtsp_media_factory_set_launch ( ahc->factory, "(ahcsrc name=camera ! tee name=t t. ! queue ! videoconvert ! videoscale ! video/x-raw, width=480, height=270, format=I420 ! x264enc tune=zerolatency ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96 t. ! queue ! videoscale ! video/x-raw, width=480, height=270 ! glimagesink name=vsink  openslessrc  ! queue ! audioconvert ! audioresample ! audiorate ! audio/x-raw, channels=1, rate=16000 ! rtpL16pay name=pay1 pt=10 )" );

    /*With preview and Audio*/
  gst_rtsp_media_factory_set_launch ( ahc->factory, "(ahcsrc name=camera ! tee name=t t. ! queue ! videoconvert ! videoscale ! video/x-raw, width=480, height=270, format=I420 ! x264enc tune=zerolatency speed-preset=superfast ! video/x-h264 ! rtph264pay name=pay0 pt=96 t. ! queue ! videoscale ! video/x-raw, width=480, height=270 !  glimagesink name=vsink  openslessrc  ! queue  ! audioconvert ! audio/x-raw, channels=1, depth=16, width=16, rate=16000 ! rtpL16pay name=pay1 pt=11 )" );

    /*Without Viewfinder preview*/
  //  gst_rtsp_media_factory_set_launch ( ahc->factory, "(ahcsrc name=camera  ! queue ! videoconvert ! videoscale ! video/x-raw, width=640, height=360, format=I420 ! x264enc tune=zerolatency speed-preset=superfast ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96   openslessrc  ! queue  ! audioconvert ! audio/x-raw, channels=1, depth=16, width=16, rate=16000 ! rtpL16pay name=pay1 pt=11 )" );

    //  gst_rtsp_media_factory_set_launch ( ahc->factory, "(ahcsrc name=camera ! tee name=t t. ! queue ! videoconvert ! videoscale ! video/x-raw, width=480, height=270, format=I420 ! x264enc tune=zerolatency ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96 t. ! queue ! videoscale ! video/x-raw, width=480, height=270 ! glimagesink name=vsink  openslessrc  ! queue  ! audioconvert ! audioresample ! audiorate! audio/x-raw, channels=1, rate=44100 ! vorbisenc ! rtpvorbispay name=pay1 pt=97 )" );

  // "( avfvideosrc ! tee name=t ! queue ! video/x-raw, width=320, height=240 ! x264enc tune=zerolatency ! rtph264pay name=pay0 pt=96 t. ! queue ! videoconvert ! osxvideosink )"
  ...
  if (ahc->native_window) {
    ...
    gst_video_overlay_set_window_handle (GST_VIDEO_OVERLAY (ahc->vsink),
      ...
      g_object_set(ahc->vsink , "sync", FALSE, NULL);
  }
